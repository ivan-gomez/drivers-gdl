...
{
	/*Se revisa el comando a ejecutar */
	switch (cmd) {

	/*Se asigna el primer número de la operación*/
	case MYCALC_IOC_SET_NUM1:
		num1 = arg;
		pr_info("NUM1: %ld\n", num1);
	break;

	/*Se asigna el segundo número de la operación*/
	case MYCALC_IOC_SET_NUM2:
		num2 = arg;
		pr_info("NUM2: %ld\n", num2);
	break;

	/*Se asigna el operando*/
	case MYCALC_IOC_SET_OPERATION:
		operation = arg;
		pr_info("OPERATION: %c\n", operation);
	break;

	/*En base a los parametros anteriormente obtenidos se obtiene el resultado
	de la operación*/
